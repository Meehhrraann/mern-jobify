...
export default function Job({_id, company, position, status, jobType, jobLocation, createdBy, createdAt, updatedAt}) {
  let date = moment(createdAt).format('MMM Do, YYYY')
  const { setEditJob, deleteJob } = useAppContext()
  return (
    <Wrapper>
      <header>
        <div className="main-icon">{company.charAt(0)}</div> {/* style first char by css */}
        <div className="info">
          <h5>{position}</h5>
          <p>{company}</p>
        </div>
